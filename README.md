...
** Check the `ConnectionStrings` in `appsettings.json` files under the `Misars.Foundation.App.HttpApi.Host` and `Misars.Foundation.App.DbMigrator` projects and change it if you need.
**
...
* Run `Misars.Foundation.App.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `Misars.Foundation.App.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `Misars.Foundation.App.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
* `Misars.Foundation.App.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
